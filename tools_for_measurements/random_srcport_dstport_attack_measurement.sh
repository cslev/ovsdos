...
function print_help {
  ...
  c_print "bold" "./random_srcport_dstport_attack_measurement.sh"
  c_print "none" "Example:"
  c_print "none" "./random_srcport_dstport_attack_measurement.sh"
  echo
  exit -1
}
...
PCAP_DIR=../pcap_generator/random_sport_dport/
RES_DIR=random_srcport_dstport_attack_measurement

PCAP_BASE="SP_DP"
RES_FILEBASE="random_srcport_dstport_attack"
...
do
  ...
  do
    ...
    python full_random_header_generator.py -n $i -ac > $PCAP_DIR/${PCAP_BASE}_${i}_${iter}.csv
