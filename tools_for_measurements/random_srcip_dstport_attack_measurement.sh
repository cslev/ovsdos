...
function print_help {
  echo
  c_print "none" "Add random allow rule on destination port and another one on source IP address to OVS with a catch-all drop rule and send n random packets generated online (n=,17,34,68,85,170,850,1700,2500, 5000, 7500, 10000, 25000, 50000). Do the whole thing 100 times!"
...
PCAP_BASE="SIP_DP"
RES_FILEBASE="random_srcip_dstport_attack"
...
do
  ...
  do
    ...
    python full_random_header_generator.py -n $i -ab > $PCAP_DIR/${PCAP_BASE}_${i}_${iter}.csv
